...
function normalizeCrmCustomer(raw: unknown): CrmCustomer | null {
  ...
  return {
    ...
    priority: oneOf(r.priority, ["low", "medium", "high", "urgent"] as const, "medium"),
    owner: str(r.owner),
    source: str(r.source),
    primaryContactName: str(r.primaryContactName),
    primaryContactTitle: str(r.primaryContactTitle),
    phone: str(r.phone),
    email: str(r.email),
    wechat: str(r.wechat),
    ...
    annualValue: str(r.annualValue),
    budget: str(r.budget),
    expectedCloseDate: str(r.expectedCloseDate),
    nextFollowUpAt: str(r.nextFollowUpAt),
    requirements: str(r.requirements),
    notes: str(r.notes),
    tags: Array.isArray(r.tags) ? r.tags.filter((x): x is string => typeof x === "string" && !!x.trim()) : [],
    solutionPlanIds,
